...
- Faster startup (min of 21 runs, source checkout): `dss version` 100 → 49 ms, `commands run` 138 → 47 ms, `dataset create --plan` 111 → 62 ms, `dataset list` 130 → 108 ms. Flag validation loads only the command registry: plan builders moved to `src/cli/plans.ts` and load only for `--plan`. The `commands run` summary reads action names from `command-syntax.json` without loading command modules. `DataikuClient` loads each resource module on first use. `node:crypto`, `node:stream`, and `node:zlib` are gone from the common paths, and `node:tls` loads only for a custom CA bundle. Registry, summary, and agent-contract output are byte-identical.
- `datasets.download` / `dss dataset download` stream through Web Streams (`TransformStream`, `CompressionStream("gzip")`) into `Bun.write`: a 200k-row gzip export takes 345 ms instead of 571 ms (same bytes), and reaching the row limit now cancels the response instead of draining it.
- Binary downloads (`writeResponseToFile`) stream with `Bun.write` into the exclusive owner-only temp file before the atomic rename. A temp file that cannot be created still cancels the response body.
