...
All B1–B27 + G1 fixed, with regression tests; full suite **597 pass / 65 skip / 0 fail** (662 tests), `tsc` clean, and key repros re-verified live against the instance. Landed as focused commits on top of a behavior-preserving refactor:
...
Re-auditing the 16 raw findings files surfaced items that were not part of the consolidated B1–B27/G1 list. Final status (suite now **597 pass / 65 skip / 0 fail**, 662 tests):
...
- **Round 2 — real-data workflow audit (2026-07-02, workflowz):** materialized real data via a `code run` bootstrap (create a managed Filesystem dataset → write rows through the Dataiku writer API; `folder upload` stays blocked by the 403) and ran 6 parallel end-to-end flow builders in isolated `ZZ_WFZ_*` scratch projects (all self-cleaned; 4 tutorials intact). Exercised SUCCESS paths prior rounds could only mark "couldn't verify":
  ...
  - **Minor observations (not defects):** `dataset download` without `--output` writes the CSV into the current directory — an ergonomic/safety caveat, not a bug (the `{path,rows}` return is self-consistent, per the not-a-defect note above); pass `--output` to avoid writing into cwd. `dataset validate-build` checks configuration only (documented). After `dataset clear`, `preview` returns a 500 rather than an empty set. After `project delete`, `project get` returns 403 rather than a clean not-found.
