...
Baseline validated: **0.10.1** (`ea60911`, pre-fix). All fixes in this report landed on top of that baseline (current `main`); the suite counts below reflect the post-fix tree. Live DSS instance (Designer node, app-scoped key `dkuaps-`), 4 tutorial projects: `DKU_EXAM_ADV_DESIGNER`, `DKU_TUT_FUZZY_JOIN`, `QS_DATA_PREP`, `QS_ML`.

## Remediation status (2026-07-01) — ALL 28 numbered findings FIXED

All B1–B27 + G1 fixed, with regression tests; full suite **603 pass / 65 skip / 0 fail** (668 tests), `tsc` clean, and key repros re-verified live against the instance. Landed as focused commits on top of a behavior-preserving refactor:
...
Re-auditing the 16 raw findings files surfaced items that were not part of the consolidated B1–B27/G1 list. Final status (suite now **603 pass / 65 skip / 0 fail**, 668 tests):
...
- **Triaged as not-a-defect:** fresh Python recipe has no payload so `recipe get-payload` errors (expected — a new recipe has no code until `set-payload`); `dataset download` returning `{path,rows,...}`+file is self-consistent (`producesLocalFile:true`) — the README `--raw` contract is recipe-payload-only. **Correction (2026-07-02):** the earlier "empty `scenario run-and-wait` times out (degenerate; DSS never signals completion)" call was WRONG — it was the run-id resolution bug below (`runAndWait` matched the trigger run id, never the resolved scenario run id), now fixed.
...
- **Follow-up live round (real `code-env` create→delete lifecycle):**
  ...
  - `refactor(code-env)` + the classification fix above: `delete --if-exists` now deletes directly and treats a `not_found` failure as the idempotent skip (dropping a redundant pre-`get`; `--dry-run` keeps the preview `get`). Net effect — `delete <missing> --if-exists` now returns a clean skip (exit 0) instead of the old exit-3 transient. **NOT fixed — inherent DSS race:** an env created with `--no-wait` and deleted `--if-exists` before DSS registers it is briefly invisible to *both* `get` **and** `list`, so `--if-exists` still skips it and it can reappear once the build finishes. No client-side existence check resolves this (a bounded poll would penalize the common truly-absent case). Not client-fixable; the post-run hygiene sweep detects and force-cleans any such leak (one was caught + cleaned this round). Regression test covers the missing-env skip only.
...
- **Round 2 — real-data workflow audit (2026-07-02, workflowz):** materialized real data via a `code run` bootstrap (create a managed Filesystem dataset → write rows through the Dataiku writer API; `folder upload` stays blocked by the 403) and ran 6 parallel end-to-end flow builders in isolated `ZZ_WFZ_*` scratch projects (all self-cleaned; 4 tutorials intact). Exercised SUCCESS paths prior rounds could only mark "couldn't verify":
  - **Closed (now pass on real data):** dataset `preview`/`download`/`schema`/`source`/`list-partitions`; `metrics dataset-compute` + `dataset-get`/`dataset-history` (real record counts); `data-quality create-rule`/compute/`status` (the prior 404-on-unbuilt did NOT reproduce on materialized data); `statistics create-worksheet`/run (the suspected dry-run persistence did NOT reproduce); a real `job build` + `job get`/`summary`/`log`/`log-url`/`wait` on a genuine build job.
  - **Bug found + fixed:** a `sync` recipe whose dataset output had an empty schema built to `DONE`/success yet produced an EMPTY output (0 rows/0 cols), silently. `fix(recipe): propagate input schema to schemaless sync outputs` copies the input schema onto the output (matching the DSS UI) and reports `syncOutputSchemaPropagated`; regression tests added.
  - **Verb corrections (no bug):** `metrics dataset-compute` (not `metrics compute`); `data-quality rules` for list; `statistics get-worksheet` (not `statistics get`); build a dataset via `job build`/`job build-and-wait` or `recipe run --wait` (there is no `dataset build`); `future` is singular (no `futures list`); `job log-url` takes a cat-activity-log URL, not a job id.
  - **Minor items triaged (live):** `dataset download` without `--output` writes into the working directory — the default path is kept (non-breaking) but the write is no longer silent: it emits a `dataset_download_default_location` warning (`feat(cli): warn when dataset download omits --output`, with test). The other two suspected rough edges are actually **correct behavior**: after `dataset clear`, `preview` returns a clean `validation_failed` (exit 2, non-retryable); after `project delete`, `project get` returns a clean `permission_denied` (exit 2) — DSS hides deleted projects. `dataset validate-build` is config-only by design.
  - **Issue-tracker triage (2026-07-02):** **#24** (`app save-instance-manifest` 500 `AssertionError` on classic app-instance projects) — FIXED + auto-closed (`fix(app)`: GET-before-PUT `projectAppType==="APP_INSTANCE"` guard + SDK `ClientValidationError` → `validation_failed`/exit 1). **PR #25** (its fix) — closed as **superseded**; its diff was stale against the module/test split, so the design was ported onto the current layout with resource + CLI regression tests. **#23** (App Designer enhancement) — its `scenario run-and-wait` false-TIMEOUT bug is FIXED (`fix(scenario)`: resolve the real run id via `get-run-for-trigger`, surfacing `triggerRunId` when it differs; verified live: trigger `…-188` vs run `…-191`); the remaining new-command requests (`app manifest export-resource`, homepage-tile helpers, `scenario get/set-script`) stay OPEN as a scoped enhancement.
...
## Verdict
The CLI is broadly solid: reads, `--fields` projection, the JSONL error contract, exit codes, idempotency flags (`--if-exists`/`--if-not-exists`), and full dataset/recipe/scenario/wiki/dashboard/insight/discussion/notebook/data-quality lifecycles work against live DSS. An end-to-end `project duplicate → job build → job get/summary/log` chain succeeded. One idempotency caveat: `code-env delete --if-exists` immediately after a `create --no-wait` can still leak (inherent DSS registration race — see the follow-up round above). But there are **4 systemic issues** and **~32 distinct defects**, two of which are dangerous for agent use.
...
- Tutorial Filesystem datasets ship unmaterialized, but this is now **mitigated**: a `code run` bootstrap (writer API) materializes a managed dataset with real rows, and `preview`/`download`/`schema`/`metrics`/`data-quality`/`statistics` all verified against it in Round 2. `QS_DATA_PREP.job_postings` (`UploadedFiles`) is materialized and previews directly. Unmaterialized datasets still fail `preview`/`download`/`list-partitions` cleanly with `validation_failed`.
