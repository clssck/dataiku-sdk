...
The CLI exposes a stable, agent-facing contract via `dss commands run` and `dss agent contract`. Treat these outputs as a compatibility boundary — do not change command names, flags, exit codes, or the contract shape without a deliberate, versioned reason.
