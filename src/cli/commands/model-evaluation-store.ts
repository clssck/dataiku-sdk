...
export const modelEvaluationStoreCommands: Record<string, CommandMeta> = withUsage(
	"model-evaluation-store",
	{
		...
		get: {
			handler: (c, a, f,) => {
				requireArgs(
					a,
					1,
					commandUsage("model-evaluation-store", "get",),
				);
		...
		create: {
			handler: async (c, _a, f,) => {
				const usage = commandUsage("model-evaluation-store", "create",);
		...
		"list-evaluations": {
			handler: (c, a, f,) => {
				requireArgs(
					a,
					1,
					commandUsage("model-evaluation-store", "list-evaluations",),
				);
		...
		delete: {
			handler: async (c, a, f,) => {
				requireArgs(
					a,
					1,
					commandUsage("model-evaluation-store", "delete",),
				);
				const projectKey = f["project-key"] as string | undefined;
