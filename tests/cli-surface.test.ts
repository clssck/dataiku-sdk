...
const EXPECTED_COMMANDS: Record<string, string[]> = {
	...
	group: ["create", "delete", "get", "list", "update",],
	plugin: [
		"code-env-create",
		"code-env-update",
		"contents-delete",
	...
	llm: ["completions", "embeddings", "list",],
	"knowledge-bank": ["clear", "search",],
	macro: ["abort", "get", "list", "result", "run", "run-and-wait", "state",],
	"data-collection": [
		"add-object",
		"create",
		"delete",
...
describe("CLI command surface", () => {
	it("exposes every supported resource/action in the machine-readable registry", async () => {
		...
		for (const action of ["create", "set-settings",]) {
			expect(registry["ml-task"][action].optionalFlags, `ml-task ${action} dry-run`,).not.toContain(
				"dry-run",
			);
		}
		// train advertises and implements a zero-HTTP --dry-run preview.
		expect(registry["ml-task"].train.optionalFlags, "ml-task train dry-run",).toContain(
			"dry-run",
		);
		expect(registry["ml-task"].train.dryRun, "ml-task train dryRun",).toBe(true,);
